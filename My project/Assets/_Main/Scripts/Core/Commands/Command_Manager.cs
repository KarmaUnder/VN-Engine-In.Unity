...
namespace COMMANDS
{
    ...
    public class Command_Manager : MonoBehaviour
    {
        ...
        private Command_Database database;
        private void Awake()
        {
            if (instance == null)
            {
                instance = this;
                database = new Command_Database();
        ...
        {
            yield return WaitingForProcessComplete(command, args);

            process = null;
        }

        private IEnumerator WaitingForProcessComplete(Delegate command, string[] args)
        {
